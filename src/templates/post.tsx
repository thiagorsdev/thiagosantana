			...
			<Helmet>
				...
				{config.twitter && (
					<meta
						name="twitter:site"
						content={`@${
							config.twitter.split("https://twitter.com/")[1]
						}`}
					/>
				)}
				{config.twitter && (
			...
			<Wrapper css={PostTemplate}>
				...
				>
					<div css={inner}>
						...
						<article
							css={[PostFull, !post.frontmatter.image && NoImage]}
						>
							<PostFullHeader>
								<PostFullMeta>
									<PostFullMetaDate
							...
							{config.showSubscribe && (
								<Subscribe title={config.title} />
							)}

							<PostFullFooter>
								<AuthorCard author={post.frontmatter.author} />
								<PostFullFooterRight
									authorId={post.frontmatter.author.id}
								/>
							</PostFullFooter>
						</article>
					</div>
...
export const query = graphql`
	query($slug: String, $primaryTag: String) {
		logo: file(relativePath: { eq: "img/tsantana.png" }) {
			childImageSharp {
				fixed {
